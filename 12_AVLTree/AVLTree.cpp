...
template <typename T>
void AVLTree<T>::print(const std::string& prefix, AVLNode<T>* node, bool isRight)
const {
if (!node) {
return;
}
...
void AVLTree<T>::rotateLeftRef(AVLNode<T>*& node) {
    ...
    // Update heights
    node->height = std::max(getHeight(node->left), getHeight(node->right)) + 1;
    left_kid->height = std::max(getHeight(right_kid->right), node->height) + 1;
    node = right_kid;
}
template <typename T>
...
template <typename T>
void AVLTree<T>::insert(const T& val, AVLNode*& node){
    if(!node){
